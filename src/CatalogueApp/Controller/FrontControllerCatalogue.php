    ...
    public function execute()
    {
        ...
        try {
            ...
            if($this->request->getSession('login') == '') {
                if($mdpAuthentif != '') {
                    if(!$authentif->checkAuthentification($loginAuthentif, $mdpAuthentif)) {
                        ...
                        $menu = array("Accueil" => '?objet=home&amp;action=makeHomePage',
                            ...
                            "Upload fichier" => '?objet=home&amp;action=upload'
                        );
                ...
                $content .= "<ul class='auth'>";
                $content .= "<li><div class='barre_auth'><label>Bonjour, " . $this->request->getSession('prenom') . " " . $this->request->getSession('nom') . "</label></div></li>";
                $content .= "<li>
                                <div class='position'>
                                    <div class='svg-wrapper'>
                                        <svg height='40' width='150' xmlns='http://www.w3.org/2000/svg'>
